...
class WeakAbstractionTest extends WordSpec with Matchers {
  ...
  "Example 2.19    --- w      [x].xy ≡ SI(Ky)               ∀ x, y" in
    ∀(varGen, varGen) { (x, y) =>
      whenever(x != y) {
  ...
  "Theorem 2.21.A  --- w      [x].M exists                  ∀ M, x" in
    ∀(varGen, termGen) { (x, M) =>
      WeakAbstraction(x, M) should not be null
    }

  "Theorem 2.21.B  --- w      ([x].M)N ▹w [N/x]M            ∀ M, N, x" in
  ...
  "Theorem 2.21.B  --- w      ([x].M)N ▹w [N/x]M            ∀ M, N, x ∈ FV(M)" in
    ∀(termGen, termGen) { (M, N) =>
  ...
  "Theorem 2.21.C  --- w      x ∉ FV([x].M)                 ∀ M, x" in
    ∀(varGen, termGen) { (x, M) =>
      val abstracted = WeakAbstraction(x, M)
      abstracted.FV should not contain x
    }

  "Theorem 2.21.C  --- w      x ∉ FV([x].M)                 ∀ M, x ∈ FV(M)" in
    ∀(termGen) { M =>
      whenever(M.FV.nonEmpty) {
        val x = M.FV.head
        val abstracted = WeakAbstraction(x, M)
  ...
  "Exercise 2.22.a --- w      [x].u(vx) ≡ S(Ku)(S(Kv)I)     ∀ u, v, x" in
  ...
  "Exercise 2.22.b --- w      [x].x(Sy) ≡ SI(S(KS)(Ky))     ∀ x, y" in
    ∀(varGen, varGen) { (x, y) =>
      whenever(x != y) {
  ...
  "Exercise 2.22.c --- w      [x].uxxv ≡ S(S(S(Ku)I)I)(Kv)  ∀ u, v, x" in
    ∀(varGen, varGen, varGen) { (u, v, x) =>
      whenever(u != v && u != x && x != v) {
