...
class PrimitiveAbstractionTest extends WordSpec with Matchers {
  ...
  "Theorem 2.21.A  --- p      [x].M exists                  ∀ M, x" in
    ∀(varGen, termGen) { (x, M) =>
      PrimitiveAbstraction(x, M) should not be null
    }

  "Theorem 2.21.B  --- p      ([x].M)N ▹w [N/x]M            ∀ M, N, x" in
  ...
  "Theorem 2.21.B  --- p      ([x].M)N ▹w [N/x]M            ∀ M, N, x ∈ FV(M)" in
    ∀(termGen, termGen) { (M, N) =>
  ...
  "Exercise 2.22.a --- p      [x].u(vx) ≡ S(Ku)(S(Kv)I)     ∀ u, v, x" in
  ...
  "Exercise 2.22.b --- p      [x].x(Sy) ≡ SI(S(KS)(Ky))     ∀ x, y" in
    ∀(varGen, varGen) { (x, y) =>
      whenever(x != y) {
  ...
  "Exercise 2.22.c --- p      [x].uxxv ≡ S(S(S(Ku)I)I)(Kv)  ∀ u, v, x" in
    ∀(varGen, varGen, varGen) { (u, v, x) =>
      whenever(u != v && u != x && x != v) {
