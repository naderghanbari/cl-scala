...
class AbstractionDSLTest extends WordSpec with Matchers {
  ...
  "Theorem 2.21.B  --- η DSL           ([x].M)N ▹w [N/x]M        ∀ M, N, x" in
  ...
  "Exercise 2.22.b --- w DSL       [x].x(Sy) ≡ SI(S(KS)(Ky))     ∀ x, y" in
    ∀(varGen, varGen) { (x, y) =>
      whenever(x != y) {
  ...
  "Exercise 2.22.c --- p DSL     [x].uxxv ≡ S(S(S(Ku)I)I)(Kv)    ∀ u, v, x" in
  ...
  "Exercise 2.25.a --- η DSL     [x,y].x ≡ K                     ∀ x, y" in
    ∀(varGen, varGen) { (x, y) =>
      whenever(x != y) {
        import Abstraction.Implicits.eta
        |(x,y)| x shouldEqual K
  ...
  "Exercise 2.25.b --- η DSL     [x,y,z].xz(yz) ≡ S              ∀ x, y, z" in
