...
trait ExpressionEval {
  ...
  def evalAbstraction(a: Abstraction)(implicit ρ: Env, abs: AbstractionStrategy): E[cl.Term] = for {
    body <- eval(a.body)
    Var(x) = a.bracket.head
    xs = a.bracket.tail.map(_.name)
  } yield ADTBracket(cl.Var(x), xs.map(cl.Var): _*)(body)

  def eval(expr: Expr)(implicit ρ: Env, abs: AbstractionStrategy): E[cl.Term] = expr match {
